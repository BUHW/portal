...
class SystemProfileForm extends TPage
{
    ...
    public function __construct()
    {
        ...
        $photo = new TImageCropper('photo');
        $password1 = new TPassword('password1');
        $password2 = new TPassword('password2');
        $login->setEditable(FALSE);

        $photo->setAllowedExtensions( ['jpg'] );
        $photo->setCropSize('200', '200');
        $photo->setImagePlaceholder(new TImage('app/images/photos/' . TSession::getValue('login') . '.jpg'));

        /*$photo->setImagePlaceholder(new TImage("fas:file-upload"));*/
        /*$photo->setAspectRatio(TimageCropper::CROPPER_RATIO_1_1);*/       
        /*$photo->disableButtonsRotate();
        $photo->disableButtonReset();
        $photo->disableButtonsScale();
        $photo->disableButtonsDrag();
        $photo->disableButtonsZoom();
        $photo->disableButtonsZoom();
        $photo->enableFileHandling()*/
